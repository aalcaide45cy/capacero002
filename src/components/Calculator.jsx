    ...
    return (
        <div className="max-w-7xl mx-auto px-4 pb-24 text-zinc-300">
            ...
            <div className="space-y-8">
                ...
                {/* Top Row: Left vs Right columns */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                    
                    {/* Left Column: Material and Electricity */}
                    ...
                    {/* Right Column: Labor and Risk & Margin */}
                    <div className="space-y-6">
                        {/* 4. Mano de Obra */}
                        <div className="bg-zinc-950 p-6 rounded-2xl border border-zinc-900 shadow-xl">
                            <div className="flex items-center gap-3 mb-6">
                ...
                {/* Middle Row: Machine wear (Full width) */}
                <div className="bg-zinc-950 p-8 rounded-2xl border border-zinc-900 shadow-xl overflow-x-auto">
                    <div className="flex items-center gap-3 mb-6">
                        <div className="p-2.5 bg-purple-900/30 rounded-lg text-purple-400 border border-purple-800/30">
                            <Clock className="w-6 h-6" />
                        </div>
                        <div>
                            <h3 className="text-lg font-bold text-white uppercase tracking-wider">Precio por Hora de Uso</h3>
                            <p className="text-sm text-zinc-400">Amortización de impresora y recambios</p>
                        </div>
                    </div>
                    
                    {/* Main wear editor layout */}
                    <div className="space-y-4">
                        {wearItems.map((item, idx) => {
                            const costPerHour = item.price / (item.life || 1);
                            return (
                                <div key={item.id} className="border-b border-zinc-900 pb-4 last:border-b-0 last:pb-0 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                    <div className="md:w-1/3">
                                        <span className="font-bold text-zinc-100 text-base md:text-lg block">{item.name}</span>
                                        <span className="text-xs text-zinc-400">{item.desc}</span>
                                    </div>
                                    <div className="flex flex-row gap-4 items-center flex-1 md:justify-end">
                                        <div className="w-32">
                                            <label className="text-xs text-zinc-400 block mb-1">Precio Reemplazo (€)</label>
                                            <input
                                                type="number"
                                                value={item.price}
                                                onChange={(e) => handleWearChange(idx, 'price', e.target.value)}
                                                className="w-full bg-zinc-900 border border-zinc-800 focus:border-capaBlue rounded px-3 py-1.5 text-white text-sm outline-none"
                                            />
                                        </div>
                                        <div className="w-32">
                                            <label className="text-xs text-zinc-400 block mb-1">Vida Útil (Horas)</label>
                                            <input
                                                type="number"
                                                value={item.life}
                                        ...
                                        <div className="text-right w-24 pt-4">
                                            <span className="text-sm text-zinc-500 block">€ / Hora</span>
                                            <span className="text-base font-bold text-zinc-200">{costPerHour.toFixed(3)} €</span>
                                        </div>
                                    </div>
                                </div>
                            );
                    ...
                    <div className="mt-6 pt-6 border-t border-zinc-900 flex justify-between items-center text-base md:text-lg">
                        <span className="text-zinc-400 font-semibold">Coste total de máquina por hora</span>
                        <span className="text-white font-black">{wearPerHour.toFixed(3)} €/H</span>
                    </div>
                    <div className="mt-2 flex justify-between items-center text-sm text-zinc-400">
                        <span>Subtotal Desgaste ({horasImpresion}H)</span>
                        <span className="font-bold text-white">{subtotalDesgaste.toFixed(2)} €</span>
                    </div>
                </div>
                ...
                <div className="flex justify-center pt-4">
                    <button
                        ...
                        className="relative flex items-center justify-center h-14 px-12 bg-gradient-to-r from-blue-700 via-blue-500 to-cyan-500 text-white font-black text-lg rounded-full transition-all duration-300 hover:scale-105 shadow-[0_0_20px_rgba(59,130,246,0.5)] hover:shadow-[0_0_30px_rgba(34,211,238,0.7)] uppercase tracking-wider overflow-hidden group cursor-pointer active:scale-95 w-full sm:w-auto"
                    >
                ...
                {resultados && (
                    <div className="bg-gradient-to-b from-zinc-900 to-zinc-950 p-8 rounded-3xl border border-zinc-800 shadow-2xl space-y-8 animate-fade-in">
                        <h3 className="text-base font-bold text-capaBlue tracking-widest uppercase border-b border-zinc-800 pb-3">
